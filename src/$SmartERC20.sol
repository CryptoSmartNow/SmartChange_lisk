...
contract SmartChange is ERC20, Ownable(msg.sender) {
    ...
    modifier onlyBridge() {
        require(msg.sender == BRIDGE, "$SMART: only bridge can mint and burn");
        _;
    }

    constructor(uint256 initialSupply, address _owner, address _bridge,address _remoteToken) ERC20("SmartChange", "$SMART") {
    ...
    function transfer(address to, uint256 amount) public override returns (bool) {
        require(_balances[address(this)] >= amount, "$SMART: insufficient token amount.");
    ...
    function transferFrom(address from, address to, uint256 amount) public override returns (bool) {
        require(_balances[from] >= amount, "$SMART: insufficient token amount.");
